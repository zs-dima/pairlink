...
- `tool/peer.dart`: a computer plays the other phone, outlet or panel, over `adb reverse`/`forward`,
  so an app is walked with one phone; `tool/phone_invite.py` reads the invite from the phone's QR.
  Not published (`.pubignore`).
